...
async function readDefaultTsConfig(fileName = exports.defaultTsConfigPath) {
    ...
    const { readConfiguration } = await Promise.resolve().then(() => __importStar(require('@angular/compiler-cli')));
...
async function initializeTsConfig(defaultTsConfig, entryPoints) {
    ...
    if (defaultTsConfigParsed.errors.length > 0) {
        const { formatDiagnostics } = await Promise.resolve().then(() => __importStar(require('@angular/compiler-cli')));
